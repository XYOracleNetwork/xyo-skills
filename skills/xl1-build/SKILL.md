...
After the user confirms (or edits) the spec, **write it to `PRD.md` at the cwd** before handing off. The PRD is the durable, agent-readable record of the plan — it survives the chat, primes future sessions, and lets [xl1-scaffold](../xl1-scaffold/SKILL.md) recover the spec if chat context is lost.
...
The PRD captures **exactly the data already in the synthesized prompt** — no new questions, no new decisions. It's a reshaping of the same content into a durable markdown file.
