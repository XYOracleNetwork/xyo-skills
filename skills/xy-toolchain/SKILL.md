...
Read when choosing among `build`, `check`, `fix`, `clean`, `deplint`, `api-exposure`, `publint`, `dead`, repository-policy commands, `skills`, or `work` (including GitHub Issues dual-write, `list --all`, and `sync`); also read when configuring dependency placement/presence, clean patterns, or using `--rules`, `--json`, `--strict`, and automation behavior.
